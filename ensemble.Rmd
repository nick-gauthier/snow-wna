...
```{r}
saveRDS(gcm_ensemble, 'gcm_ensemble.rds')
```
...
```{r}
n_modes <- 7 # is this the right number?
prism_patterns <- get_patterns(filter(prism, time <= 2010), k = n_modes)
prism_recon <- reconstruct_field(prism_patterns)
prism_recon2 <- predict_cca(get_patterns(filter(cera, time >= 1982), 10), 
            get_patterns(filter(prism, time <= 2010), 9), 
            filter(cera, time >= 1982),
            k = 7)
```

```{r}
resids <- (prism_recon - filter(prism, time <= 2010)) %>%
  setNames('error')

resids2 <- (prism_recon2 - filter(prism, time <= 2010)) %>%
  setNames('error')

  mutate(error_pearson = error / st_apply(filter(prism, time <= 2010), 1:2, sd), # pearson residuals
        error_stand = error / sd(error)) # standardized (internally studentized)
```

```{r}
plot(st_apply(resids, 1:2, sd));plot(st_apply(resids2, 1:2, sd))
```

```{r}
error_sd <- st_apply(resids2, 1:2, sd)
slice(prism, 'time', 1)
st_apply(resids, 1:2, sd)
prism_patterns$eofs
prism

plot(resids / error_sd) 
```


```{r}
ggplot() + geom_stars(data = resids) + facet_wrap(~time) + geom_sf(data = states_wus, fill = NA) + scale_fill_distiller(palette = 'RdBu', na.value = NA, limits = c(-712, 712))
```

```{r}
# i think this causes some divide by zeroes?
pts <- (resids2 / error_sd) %>%
  slice('time', 1) %>%
  st_as_sf(as_points = TRUE) %>%
  remove_missing() %>%
  st_coordinates()

dat <- (resids2 / error_sd) %>%
  as_tibble() %>% 
  remove_missing() %>%
  pivot_wider(id_cols = c(x,y), names_from = time, values_from = error) %>%
  select(-x, -y) %>%
  units::drop_units()
...
```{r}
out1<- LatticeKrig(pts, dat)
out2<- LatticeKrig(pts2, dat2)
```
...
reps <- 16
gSim <- LKrig.sim(pts, out1$LKinfo, M = reps) %>% 
  `colnames<-`(1:reps) %>%
  cbind(pts, .) %>% 
  as_tibble() %>% 
  rename(x = X, y = Y) %>%
  pivot_longer(-c(x,y), names_to = 'rep', values_to = 'error') %>% 
  st_as_stars(dims = 1:3) %>% 
  st_set_crs(4326) # * sqrt(out1$rho.MLE.FULL)

gSim2 <- LKrig.sim(pts2, out2$LKinfo, M = reps) %>% 
  `colnames<-`(1:reps) %>%
  cbind(pts2, .) %>% 
  as_tibble() %>% 
  rename(x = X, y = Y) %>%
  pivot_longer(-c(x,y), names_to = 'rep', values_to = 'error') %>% 
  st_as_stars(dims = 1:3) %>% 
  st_set_crs(4326) # * sqrt(out2$rho.MLE.FULL)
```

```{r}
st_apply(gSim, 1:2, sd) %>% plot
st_apply(gSim2, 1:2, sd) %>% plot
...
```{r}
ggplot() +
  geom_stars(data = gSim) +
  facet_wrap(~rep) +
  scale_fill_viridis_c(na.value = NA) +
  coord_quickmap() +
  theme_bw()
ggplot() +
  geom_stars(data = gSim2) +
  facet_wrap(~rep) +
  scale_fill_viridis_c(na.value = NA) +
  coord_quickmap() +
  theme_bw()
...
ggplot() +
  geom_stars(data = gSim * error_sd) +
  facet_wrap(~rep) +
  scale_fill_viridis_c(na.value = NA) +
  coord_quickmap() +
  theme_bw()
ggplot() +
  geom_stars(data = gSim2 * error_sd) +
  facet_wrap(~rep) +
  scale_fill_viridis_c(na.value = NA) +
  coord_quickmap() +
  theme_bw()
...
ggplot() +
  geom_stars(data = (resids2 / error_sd)[,,,1:16]) +
  facet_wrap(~time) +
  scale_fill_viridis_c(na.value = NA) +
  coord_quickmap() +
  theme_bw()
...
```{r}
times <- st_get_dimension_values(recon_test, 'time')

 
ccsm_ensemble <- map(1:15, ~LKrig.sim(pts2, out2$LKinfo, M = 1000) %>% 
  `colnames<-`(times) %>%
  cbind(pts2, .) %>% 
  as_tibble() %>% 
  rename(x = X, y = Y) %>%
  pivot_longer(-c(x,y), names_to = 'time', values_to = 'error', names_transform = list(time = as.integer)) %>% 
  st_as_stars(dims = 1:3) %>% 
  st_set_crs(4326) %>%
   `*`(error_sd) %>%
   `+`(units::drop_units(recon_test))) %>% 
  append(list(along = 'ensemble')) %>%
  do.call(c, .) %>%
    setNames('swe')# %>%
    mutate(swe = if_else(swe > 0, swe, 0))
```

```{r}
new_test <- ccsm_ensemble
```
...
```{r}
sim <- LKrig.sim(pts2, out2$LKinfo, M = reps) %>% 
  `colnames<-`(1:reps) %>%
  cbind(pts2, .) %>% 
  as_tibble() %>% 
  rename(x = X, y = Y) %>%
