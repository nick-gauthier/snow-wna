...
swe_dat %>%
  ...
  coord_quickmap() +
  geom_sf(data = states_wus, fill = NA, color = 'white', color = 1.2) +
  theme_void() +
...
## Comarison to Paleo Simulations
...
cesm_dat %>%
  ...
  ggplot() +
  geom_raster(aes(x, y, fill = SWE)) +
  geom_sf(data = states_wus, fill = NA, colour = 'white', color = 1.2) +
  scale_fill_viridis_c() +
  coord_sf()+
  theme_void() +
...
swe_dat%>%
  ...
  geom_sf(data = states_wus, fill = NA, color = 'black', color = 1.2) +
  facet_wrap(~PC) +
