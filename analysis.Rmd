...
left_join(prism_resamp, mean_swe_ccsm_ext, by = c('x', 'y')) %>%
  gather(type, SWE, 3:4) %>%
ggplot() +
  geom_raster(aes(x, y, fill = SWE)) +
  geom_sf(data = states_wus, fill = NA, color = 'white') +
...
```{r calc_eofs}
prism_eofs <- get_eofs(prism_pca, prism_eigs, 9)
sim_eofs <- get_eofs(sim_pca, sim_eigs, 9)
