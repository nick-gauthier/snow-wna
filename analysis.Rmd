...
sree_plot(obs_eigs, 9)
```


```{r plot_variance_sim}
scree_plot(sim_eigs, 9)
```
...
```{r}
eof_plot(prism_dat, eofs_obs)
```


```{r, echo = FALSE}
eof_plot(ccsm_dat, eofs_sim)
```
...
```{r}
era <- (brick('data/era-interim_snow.nc', varname = 'rsn') * 
  brick('data/era-interim_snow.nc', varname = 'sd')) %>%
  projectRaster(ccsm)
...
```{r}
era_pca <- calc_pcs(era_dat)
era_eigs <- get_eigs(era_pca, era)
```

```{r, echo = FALSE}
plotnScree(nScree(era_eigs$eigenvalues))
```

```{r}
era_eofs <- get_eofs(era_pca, era_eigs, 9)
```
...
ggplot(aes(x = PC, y = percent * 100)) +
 ...
 geom_text(aes(x = PC, y = cumvar_line, label = paste0(round(cumulative * 100, 0), '%')), size = 2.5, vjust = 0) +
  labs(x = "Principal Component", y = "Normalized Eigenvalue") + 
  geom_vline(xintercept = 9 + .5, linetype = 2, color = 'red', alpha = .7) +
  theme_bw() + 
  guides(color = F) + 
  scale_x_continuous(breaks = seq(0, 25, 5))
```
...
```{r plot_variance_obs, echo = FALSE}
scree_plot(comb_eigs, 9)
```

```{r, echo = FALSE}
eof_plot(prism_dat, eofs_obs)

eof_plot(era_dat, era_eofs)
...
pcs_era <- get_pcs(era_pca, 9)
pcs_comb <- get_pcs(comb_pca, 9)
```
