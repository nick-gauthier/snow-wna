...
```{r}
bbox <- extent(c(-125, -100, 30, 50))
```

Import the snow observation data.^[What's up with this warning message? long_name=CRS definition
...
swe_obs <- list.files(pattern = 'SWE_Depth', full.names = TRUE) %>%
...
snow_dat <- left_join(swe_dat, depth_dat, by = c('x', 'y', 'water_year')) %>%
