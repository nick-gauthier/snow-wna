...
preprocess <- function(x, var) {
  ...
  # find the index corresponding to april 1st
  maps <- brick(x, varname = var)
  times <- getZ(maps)
  index <- which(times == as.Date(paste0(year, '-04-01')))
    
  # extract, crop, and resample
  maps %>%
    .[[index]] %>%
    crop(bbox) %>%
...
swe_dat %>%
  filter(SWE > 0) %>%
ggplot(aes(x = SWE, y = as.factor(year), fill = ..x..)) +
...
swe_dat %>%
  ...
  coord_quickmap()+
  theme_void() +
  ggtitle("April 1st, {closest_state}") +
