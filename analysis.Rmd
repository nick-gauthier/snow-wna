...
list.files('data', pattern = 'SWE_Depth', full.names = TRUE)[1] %>%
  brick %>%
  subset(t1) %>% 
  crop(bbox) %>%
    mean %>%
...
```{r}
livneh <- preprocess('~/Downloads/LIVNEH/swe.mon.mean.nc', 
                   var = 'swe', 
                   flip = TRUE, 
                   monthly = TRUE)
...
livneh_eigs <- livneh_pca %>%
  tidy(matrix = 'pcs') %>%
  mutate(eigenvalues = std.dev ^ 2,
         error = sqrt(2 / n_effective(livneh)),
         low =  eigenvalues * (1 - error) * 100 / sum(eigenvalues),
         hi = eigenvalues * (1 + error) * 100 / sum(eigenvalues),
         cumvar_line = hi + 0.02 * max(hi))
...
```{r plot_variance_livneh, fig.width = 5, fig.height = 4, echo = FALSE}
livneh_eigs %>% 
    filter(PC <= 20) %>%
ggplot(aes(x = PC, y = percent * 100)) +
  geom_errorbar(aes(x = PC, ymin = low, ymax = hi), width = 0.4) +
...
eofs_livneh <- livneh_pca %>% # calculate unrotated EOFs
  tidy(matrix = 'variables') %>%
  filter(PC <= 9) %>%
  left_join(livneh_eigs[1:2], by = 'PC') %>%
  mutate(eof = value * std.dev,
         PC = as.character(PC)) %>%
  dplyr::select(-std.dev,-value) 
```
