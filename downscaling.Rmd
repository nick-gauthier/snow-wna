...
```{r}
tidy(era_pca) %>%
  mutate(time = parse_date(str_sub(row, 2), format = '%Y.%m.%d')) %>%
  filter(PC <=6) %>%
...
era %>%
  ...
  theme_void()+
  coord_quickmap() +
  ggtitle('Simulated Precipitation EOFs')
```
...
test <- prism %>%
  ...
  mutate(time = str_split(layer, pattern = '_'),
         time = map_chr(time, ~.[5])) %>%
  mutate(time = parse_date(time, format = '%Y%m')) %>%
  select(-layer) %>%
  left_join(pcs) 
```
...
mod1 <- bam(value ~ te(x, y, by = PC1) + te(x, y, by = PC2) + te(x, y, by = PC3) + te(x, y, by = PC4) + PC1 + PC2 + PC3 + PC4, data = test)
...
test %>%
  mutate(pred = predict(mod1, .),
         res = pred - value) %>%
  filter(time < as.Date('1982-01-01')) %>%
...
test2 <- test %>%
  group_by(x,y) %>%
  nest %>%
  mutate(mod = future_map(data, ~lm(value ~ PC1 + PC2 + PC3 + PC4, data = .), .progress = TRUE),
