...
resids <- prism_recon %>%
  ...
  mutate(error = SWE_recon - SWE_obs,
           ...
           error_sd2 = error / sd(error), # standaridized (internally studentized)
...
## Lattice Krig
interesting, lattice kriging the standardized and unstandardized doesn't seem to make a difference. comparing auto a weight selection to defaults, default has fewer degrees of freedom and a rho value that's significantly larger than otherwise. fit awght of 8.4
```{r}
...
gSim <- LKrig.sim(st_coordinates(pts), out1$LKinfo, M = 6)
gSim2 <- LKrig.sim(st_coordinates(pts), out2$LKinfo, M = 6)


resids %>%
  group_by(x,y) %>%
  summarise(swe_sd = sd(error)) %>%
  bind_cols(as_tibble(gSim3)) %>%
  pivot_longer(V1:V6) %>%
    ggplot(aes(x, y)) +
