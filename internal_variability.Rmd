...
vars <- resids %>%
  ...
  group_nest(year) %>%
  mutate(var = future_map(data, ~variogram(error_sd ~ 1, .), .progress = TRUE),
         fit = future_map(var, ~fit.variogram(., vgm(c("Exp", "Mat", "Sph", "Gau", "Ste", "Cir", "Lin", "Bes"))))) 


map2(vars$var, vars$fit, ~plot(.x, model = .y))
```
```{r}
...
t2 <- vars$var[[1]]
t2$gamma <- t1$gamma
v1 <- fit.variogram(t2, vgm(c("Exp", "Mat", "Sph", "Gau", "Ste", "Cir", "Lin", "Bes")))
```
so the average variogram gets us a nugget with psil 0.09413353 and range 0, matern covariance with psill 0.37283848 and range 160.5867.
```{r}
pts <- resids %>%
  filter(year == 1982) %>%
  st_as_sf(coords = c('x','y'), crs = 4326) %>%
...
huh, if p < 0.05 that suggests it isn't normal
```{r}
resids %>%
  group_by(x, y) %>%
...
```{r}
library(LatticeKrig)
# should set to LKSphere using setup
testk <- LatticeKrig(st_coordinates(pts), pts$error_sd)
testk2 <- LatticeKrig(st_coordinates(pts), pts$error)
LKrig.cov.plot(testk2$LKinfo)
...
gSim <- LKrig.sim(st_coordinates(pts), testk$LKinfo, M = 5)
gSim2 <- LKrig.sim(st_coordinates(pts), testk2$LKinfo, M = 5)
...
## data.frame
test2 <- ~ 1 + M
(ca20.fit02.RF <- RFfit(test2, data=testdat, M=M))
